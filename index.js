...
// A: The nested function can access the variable because an inner function can access the variable internal from outside of it's scope.
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(arr){
    const displayNames = [];
    arr.forEach(function(indexVal){
      displayNames.push(`name: ${indexVal['animal_name']}, scientific: ${indexVal['scientific_name']}`)
    });
     return displayNames;
    }
  ...
  */
  ...
  function lowerCaseNames(arr) {
    let lowerName = arr.map(name => name.animal_name.toLowerCase())
    return lowerName;
    }
  ...
  */
  ...
  function lowPopulationAnimals(arr){
    const lowPop = arr.filter(zoo => zoo.population < 5);
    return lowPop;
  }
  ...
  */
  ...
  function USApop(arr){
    let pop = arr.reduce((acc, currentValue) => {
